...
export default {
  ...
  computed: {
    ...
    symptomscore: {
      get() {
        if (this.area[this.$attrs.body].SymptomScore.Lichenification === '0') {
          return this.selected;
        }
        return this.area[this.$attrs.body].SymptomScore.Lichenification;
      },
      set(val) {
        this.selected = true;
        this.$store.commit('setSymptomScore', { body: this.$attrs.body, symptom: 'Lichenification', val });
      },
    },
  },
  methods: {
  ...
  watch: {
    selected() {
      this.$store.commit('setSymptomCompleted', { body: this.$attrs.body, symptom: 'LICHENIFICATION', completed: true });
      this.$store.commit('checkSymptomCompleted', { body: this.$attrs.body });
    },
  },
  created() {
    this.grids = generateGrids('Lichenification', 'Head & Neck');
  },
};
</script>
