...
import ArmFrontImage from 'assets/images/arm-front.svg';
import ArmBackImage from 'assets/images/arm-back.svg';
...
export default {
  ...
  data() {
    return {
      ...
      tabData: {
        ...
        Lichenification: {
          num: 4,
          name: 'Lichenification', // Lichenification
          name2: 'Biceps II',
          id: '3',
  ...
  created() {
    this.gridData = generateGrids('Erythema', 'UpperExtremties');
  },
  activated() {
    window.scrollTo(0, 0);
  },
  deactivated() {
    const upperData = {
      upperAreaScore: this.areaPoint,
      upperAreaPercent: this.input,
      upperErythema: this.tabData.Erythema.score,
      upperEdema: this.tabData['Edema / papulation'].score,
      upperExcoriation: this.tabData.Excoriation.score,
      upperLichenification: this.tabData.Lichenification.score,
      upperScore: this.summary,
    };
    this.$store.commit('patient/SAVE_UPPER_DATA', upperData);
  },
  methods: {
    ...
    changeTab(tabItem) {
      ...
      // 換資料注入不同圖片跟內文
      this.gridData = generateGrids(tabItem.name, 'UpperExtremties');
...
<template>
  <div class="upper-extremities-section">
    <Modal>
    ...
    <div class="graph-section">
      <div class="front">
        ...
        <img :src="ArmFrontImage" />
      </div>
      <div class="back">
        <div class="title">Back</div>
        <img :src="ArmBackImage" />
      </div>
    </div>
